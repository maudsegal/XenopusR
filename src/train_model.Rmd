...
confusion_matrix <- table(Actual = trainingdata$class, Predicted = ml.model.svm$fitted)
...
confusion_matrix_svm <- as.data.frame(confusion_matrix)%>% 
  mutate(model_type = "SVM")

confusion_matrix <- rbind(confusion_matrix_rf, confusion_matrix_svm) %>% 
  pivot_wider(names_from = model_type, values_from = Freq)
```
